...
import FamiliasProductividad from "../Familias/familia";

export default function Origen({ onVolver }) {
  const dataOrigen = historicalData.find(item => item.seccion === "origen-nombre");
  ...
  return (
    <>
      {!mostrarSiguiente ? (
        ...
        <FamiliasProductividad onVolver={() => setMostrarSiguiente(false)} />
      )}
    </>
  );
}