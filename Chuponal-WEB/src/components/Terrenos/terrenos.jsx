...
import ViasSaludEducacion from "../Vias/vias";

export default function TerrenosServicios({ onVolver }) {
  const dataSeccion = historicalData.find(item => item.seccion === "terrenos-servicios");
  ...
  return (
    <>
      {!mostrarSiguiente ? (
        ...
        <ViasSaludEducacion onVolver={() => setMostrarSiguiente(false)} />
      )}
    </>
  );
}