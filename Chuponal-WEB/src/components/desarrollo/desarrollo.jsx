...
import TradicionesFe from "../Tradiciones/tradiciones";

export default function DesarrolloEstudiantil({ onVolver }) {
  const dataSeccion = historicalData.find(item => item.seccion === "desarrollo-estudiantil");
  ...
  return (
    <>
      {!mostrarSiguiente ? (
        ...
        <TradicionesFe onVolver={() => setMostrarSiguiente(false)} />
      )}
    </>
  );
}